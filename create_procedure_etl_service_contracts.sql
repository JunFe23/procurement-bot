...
BEGIN
  ...
  SELECT
    ...
    MAX(CASE WHEN g.rn = 1 THEN g.public_procurement_category   END),
    MAX(CASE WHEN g.rn = 1 THEN g.public_procurement_category_mid END),
    MIN(g.contract_date),
  ...
  FROM (
    SELECT
      ...
      f.detail_item_code,
      f.detail_item_name,
      f.public_procurement_category,
      f.public_procurement_category_mid,
      f.contract_date,
      f.contract_amount,
      f.is_long_term,
